...
int			mouse_controls(int event, int x, int y, t_env *env)
{
	...
	k = 1;
	if (event == MOUSE_UP && !env->start)
	{
		env->depth += 3;
		k = (double)9 / 10;
	}
	else if (event == MOUSE_DOWN && !env->start)
	{
		if (env->depth >= 20)
			env->depth -= 3;
		if (env->delta_im < 4.4)
	...
	env->delta_im *= k;
	env->delta_re *= k;
	env->re_min = env->re_min + ((double)x / WIN_WIDTH) *
		env->delta_re / k * (1 - k);
	env->im_min = env->im_min - ((double)(WIN_HEIGHT - y) / WIN_HEIGHT) *
		env->delta_im / k * (1 - k);
	draw(env);
	return (0);
}

static void	additional_keys(int key, t_env *env)
{
	if (key == KEY_C)
